...
- No published file is larger than $400$ KB, so all of it renders in the GitHub web interface. A band above that size is written as `<band>.part01.yaml`, `<band>.part02.yaml` and so on. The split falls on an entry boundary where it can; where a single matrix is itself too large, its body is continued across parts and each piece carries `matrix_parts` and `matrix_part`. [`bigfatmatrix.py`](bigfatmatrix.py) merges the pieces, so an entry spread over seven files is still read as one `matrix`.

- Entries within each YAML file are in alphabetical order by key.

- Every square matrix in the catalogue is non-singular. Renders of every entry are collected in [`visualisation.md`](visualisation.md).
...
| `571bit.part01.yaml` to `571bit.part02.yaml` | 2 | $571$ | [`571bit.md`](571bit.md) |
| `1024bit.part01.yaml` to `1024bit.part03.yaml` | 1 | $1024$ | [`1024bit.md`](1024bit.md) |
| [`1280bit.yaml`](1280bit.yaml) | 1 | $1280$ | [`1280bit.md`](1280bit.md) |
| `1600bit.part01.yaml` to `1600bit.part07.yaml` | 1 | $1600$ | [`1600bit.md`](1600bit.md) |
...
| [`rectangular256bit.yaml`](rectangular256bit.yaml) | 2 | $129$ to $256$ | [`rectangular256bit.md`](rectangular256bit.md) |
| [`rectangular325bit.yaml`](rectangular325bit.yaml) | 1 | $325$ | [`rectangular325bit.md`](rectangular325bit.md) |
| [`rectangular465bit.yaml`](rectangular465bit.yaml) | 1 | $465$ | [`rectangular465bit.md`](rectangular465bit.md) |
| [`rectangular511bit.yaml`](rectangular511bit.yaml) | 1 | $511$ | [`rectangular511bit.md`](rectangular511bit.md) |
| [`rectangular565bit.yaml`](rectangular565bit.yaml) | 1 | $565$ | [`rectangular565bit.md`](rectangular565bit.md) |
| `rectangular1141bit.part01.yaml` to `rectangular1141bit.part04.yaml` | 1 | $1141$ | [`rectangular1141bit.md`](rectangular1141bit.md) |
| `rectangular2047bit.part01.yaml` to `rectangular2047bit.part06.yaml` | 1 | $2047$ | [`rectangular2047bit.md`](rectangular2047bit.md) |

### Bit-Permutations

| File | # Entry | Index |
|:------|:-------:|:------|
| [`permutations.yaml`](permutations.yaml) | 21 | [`permutations.md`](permutations.md) |
...
Each YAML entry under a matrix file has the following fields in order (mandatory fields marked with *):
...
| `aliases` | tuple[str] | Alternative names for the same matrix used in the literature, including the identifier a public circuit corpus files it under |
| `reuse` | tuple[str] | Ciphers that use this matrix under a name of their own, without renaming it into a variant |
| `rows`<sup>*</sup> | int | Number of rows (output bit width) |
| `cols`<sup>*</sup> | int | Number of columns (input bit width) |
| `size` | int | Square size when the matrix is augmentation-only (no body) |
...
| `formula` | str | Algorithmic description of how the matrix is constructed (in pseudo-code, formulas, or rotation/shift recipes); emitted as a YAML literal block scalar (`\|-`) when multi-line |
| `augmentation` | str | Symbolic expression building this matrix from other catalogued entries (e.g., `block_diag(...)`, `kron(...)`); see Notes 7 |
| `involution` | bool | True iff $M^2 = I$ over $\mathrm{GF}(2)$ |
| `symmetric` | bool | True iff $M = M^\top$ |
| `rank` | int | Rank of $M$ over $\mathrm{GF}(2)$ |
| `is_invertible` | bool | True iff $M$ has full rank |
| `order` | int | Multiplicative order of $M$ in $\mathrm{GL}(n, \mathrm{GF}(2))$ when invertible (absent for sizes $> 320$) |
...
| `orbit` | int | Number of orbits on the $N$ rows of the smallest shift $\sigma\colon i \mapsto i + k \pmod N$ that commutes with $M$; see Notes 10 |
| `hamming_weight_per_row` | tuple[int] | Number of $1$ entries per row |
| `fixed_points` | tuple[int] | Indices $i$ with $M e_i = e_i$ (only computed for square matrices) |
| `cycle_lengths` | tuple[int] | Cycle structure when $M$ acts as a bit-permutation |
| `disjoint_cycles` | tuple | Explicit disjoint-cycle decomposition (bit-permutation matrices only) |
| `matrix`<sup>*</sup> | tuple[str] | One bit-string per row; character `c[j]` is the bit in column $j$. Absent for `augmentation`-only entries |
| `trivium` | str | Lesser-known aside about the entry, such as the etymology of the cipher's name; never a cryptographic property |
| `alias` | str | Present instead of a body when the key resolves to another entry, which is where the matrix is held |
| `matrix_parts`, `matrix_part` | int | Present only when a body is continued across part files; see Organisation |

### Permutation entry

Each YAML entry under `permutations.yaml` has the following fields in order (mandatory fields marked with *):

| Field | Type | Description |
|-------|------|-------------|
| `canonical_name`<sup>*</sup> | tuple | Name as written in the original paper, as (family, layer) |
| `aliases` | tuple[str] | Alternative names |
| `size`<sup>*</sup> | int | Number of bits permuted |
| `year`<sup>*</sup> | tuple | Significant publication years |
| `cipher`<sup>*</sup> | bool | True iff the entry is the linear layer of a primitive |
| `origin`<sup>*</sup> | str | URL of the original publication or specification |
| `source` | str | URL of code or related resource whence some information is mined |
| `note` | str | Free-form remark |
| `formula` | str | Algorithmic description of how the permutation is generated (e.g., closed-form, look-up convention) |
| `involution` | bool | True iff the permutation is its own inverse |
| `fixed_points` | tuple[int] | Indices fixed by the permutation |
| `cycle_lengths` | tuple[int] | Sorted cycle-length multiset |
| `order` | int | Lowest $k$ with $P^k = \mathrm{id}$ |
| `perm`<sup>*</sup> | tuple[int] | Permutation $P$, with `P[i]` the destination index of input bit $i$, that is, `output[P[i]] = input[i]` |

### Notes

1. A word of $n$ bits is numbered from the most significant, so bit $0$ of a $32$-bit word carries weight $2^{31}$. This is how the specifications write it, and it is what fixes the matrix of an entry defined by rotations: read the other way round, a left rotation becomes a right one and the matrix comes out transposed. `ZUC.L2` and `SM4`, for instance, are transposes of one another because their rotation offsets are negatives modulo $32$.
...
10. `SHA2.SMALLSIGMA0` and `SHA2.SMALLSIGMA1` are the FIPS 180-4 matrices, whose third term is a shift ($\mathrm{SHR}^3$ and $\mathrm{SHR}^{10}$) rather than a rotation. Several circuit corpora implement a rotate-only variant under those names; that variant is a different matrix and is not catalogued here.

11. A matrix that turns out to be catalogued under two keys is kept under one of it, chosen as the earlier publication; the other key either resolves to it through `alias` or is retired outright. [`visualisation.md`](visualisation.md) lists every such key. `SKOP15.IS16` is retired in favour of `JOLTIK`, and `MIDORI` resolves to `PRIDE.L0`.

12. `orbit` stands in for the higher-field structure of the matrix. Let $k$ be the smallest positive shift with $\sigma_k\colon i \mapsto i + k \pmod N$ commuting with $M$ on both rows and columns. Minimality forces $k$ to divide $N$, so $k$ is at once the shift and the number of orbits of $\sigma_k$ on the $N$ rows, which is the value recorded; $\sigma_k$ then has order $N / \mathrm{orbit}$. An orbit count of $N$ means no nontrivial shift commutes, hence no such structure. For `AES.MIXCOLUMN` the value is $8$, the byte rotation $i \mapsto i + 8$ of the underlying $\mathrm{GF}(2^8)$ MDS matrix, giving $8$ orbits of length $4$.
...
3. `find('pattern')` and `bigfatmatrix['pattern']` support `*` and `?` wildcards (case-insensitive) and always return a `dict`. For an exact lookup, `bigfatmatrix['KEY']` behaves identically to attribute access.

4. The loader is lazy. Importing `bigfatmatrix` does not parse any YAML file immediately; the data files are read on first access, and a band published as several parts is merged there.

5. Attribute access hands out tuples, never lists, so an entry cannot be mutated by accident. The `yaml` proxy returns the raw parsed dictionary, where a sequence is still a list.
