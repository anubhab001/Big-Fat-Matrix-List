...
Public collection of GF(2) matrices used in cryptography

Last update: 10 August 2026 <!-- TODO: To be updated in UTC with each (major) commit/push -->
...
- A key carries the layer name whenever the specification gives the layer one, and stands as the bare cipher name only when it does not. `AES.MIXCOLUMNS` is a layer of AES; `SM4.L`, `ANUBIS.H`, `KHAZAD.H`, `GROESTL.MIXBYTES`, `WHIRLPOOL.MIXROW`, `SKINNY.MIXCOLUMN` and `TWOFISH.MDS` are named the same way, each after what its own designers call it. Where a cipher has one linear layer and never names it, the key is the cipher alone, as with `PRESENT` and `TRIFLE`. Where the layer belongs to a paper rather than to a cipher, the first element is the authors: `KLSW17.S8GF4` is Kranz, Leander, Stoffelen and Wiemer; `SKOP15.S4GF8` is Sim, Khoo, Oggier and Peyrin.

- A matrix is held under the first design that publishes it, and every later use of it is a `reuse` on that entry rather than a second copy. Where the first publication is a paper on matrices and a cipher later adopts one, the entry goes to whichever of the two prints the matrix outright: `SATURNIN.MIXCOLUMN` is held under the cipher although Duval and Leurent proposed the matrix first, because their paper gives a construction and the Saturnin specification gives the matrix, and the `note` credits the earlier proposal.
...
|:------|:-------:|:-------------|:------|
| [`square/16bit.yaml`](square/16bit.yaml) | 36 | $4 \times 4$, $8 \times 8$, $16 \times 16$ | [`square/16bit.md`](square/16bit.md) |
| [`square/32bit.yaml`](square/32bit.yaml) | 54 | $20 \times 20$, $24 \times 24$, $25 \times 25$, $28 \times 28$, $32 \times 32$ | [`square/32bit.md`](square/32bit.md) |
| [`square/64bit.yaml`](square/64bit.yaml) | 43 | $48 \times 48$, $50 \times 50$, $60 \times 60$, $64 \times 64$ | [`square/64bit.md`](square/64bit.md) |
| [`square/128bit.yaml`](square/128bit.yaml) | 19 | $100 \times 100$, $127 \times 127$, $128 \times 128$ | [`square/128bit.md`](square/128bit.md) |
| [`square/256bit.part01.yaml`](square/256bit.part01.yaml), [`square/256bit.part02.yaml`](square/256bit.part02.yaml) | 11 | $163 \times 163$, $192 \times 192$, $200 \times 200$, $233 \times 233$, $256 \times 256$ | [`square/256bit.md`](square/256bit.md) |
...
| [`square/384bit.yaml`](square/384bit.yaml) | 2 | $384 \times 384$ | [`square/384bit.md`](square/384bit.md) |
| [`square/400bit.yaml`](square/400bit.yaml) | 1 | $400 \times 400$ | [`square/400bit.md`](square/400bit.md) |
| [`square/512bit.yaml`](square/512bit.yaml) | 1 | $512 \times 512$ | [`square/512bit.md`](square/512bit.md) |
...
| File | # Entry | Index |
|:------|:-------:|:------|
| [`permutations.yaml`](permutations.yaml) | 43 | [`permutations.md`](permutations.md) |
...
| `rank` | int | Rank of the matrix over `GF(2)`, the number of rows that are independent of the rest |
